...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const matchedUserId: string = record.matched_user_id;

    if (!matchId || !matchedUserId) {
      return new Response(
        JSON.stringify({ error: 'Missing match id or matched_user_id' }),
    ...
    if (!profile.phone_number) {
      console.log('No phone number for user:', matchedUserId);
      return new Response(
    ...
    const message = 'You have a new RydMatch match! Open the app to connect.';
    ...
    console.error('new-match-sms error:', err);
    return new Response(
